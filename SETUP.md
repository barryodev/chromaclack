...
- `.rs` files in `api/` are picked up by Vercel's **native Rust runtime**, so no
  ...
  pin it.)
- The project's GitHub integration must actually be connected (Project Settings
  → Git) for pushes to trigger deployments — linking the project locally via
  `vercel link`/`vercel deploy` is not the same thing and won't set up CI/CD.
...
One-time setup, via Android Studio's SDK Manager:

1. Install the **Android SDK Command-line Tools** (SDK Manager → SDK Tools tab)
   and the **NDK (Side by side)** component.
2. Install a JDK 17 and set `JAVA_HOME` (e.g. `sudo apt install openjdk-17-jdk`).
3. Add the mobile target(s) you need in `rust-toolchain.toml` (uncomment
   `aarch64-linux-android` for a real arm64 phone) and run `rustup target add
   aarch64-linux-android`.
4. On the phone: Settings → About Phone → tap the build number 7 times to
   unlock Developer Options, then enable **USB debugging**.
5. `pnpm --filter @repo/desktop android:init` once, to generate
   `src-tauri/gen/android`.

Day to day:

```bash
pnpm --filter @repo/desktop android:dev
```

This runs `tauri android dev --host <LAN IP>`, where the IP is resolved at run
time (`ip route get 1.1.1.1`) instead of hardcoded — useful since most home
networks don't hand out a static DHCP lease. `TAURI_DEV_HOST` is wired through
`vite.config.ts` so the dev server binds correctly for a physical device to
reach it over the LAN.

**This only works if the phone and the PC are on the same Wi-Fi network**, and
if the PC's firewall allows inbound connections on the dev server port (`5173`)
and the Vite HMR port (`5174`) from the LAN. If the app can't reach the dev
server, isolate the problem before assuming Tauri is broken: start `pnpm
--filter @repo/frontend dev -- --host <LAN IP>` and open that URL in the
phone's own browser. If the page doesn't load there either, it's a network/
firewall issue, not a Tauri one.

Two USB gotchas from experience: a charge-only cable produces a flaky
`adb devices` (`offline`/disconnects); and Android Studio running in the
background hijacks the `adb` server the CLI needs, so close it before running
`android:dev` from the terminal.

Note: `local-api.js` (the standalone Node server behind the `/api` proxy in
`vite.config.ts`) is **not** needed for the Tauri app itself — inside a Tauri
webview `isTauri()` is true, so the environment adapter always uses the
`invoke()` IPC path, never `fetch('/api/greet')`. It's only relevant if you're
testing the plain browser page (e.g. as a network isolation check, above).
...
`.github/workflows/build-linux.yml` builds the native Ubuntu/Linux executable on
every push/PR to `main` and uploads it as a build artifact. It installs the same
system dependencies listed under Prerequisites, then runs `pnpm build:desktop`.
